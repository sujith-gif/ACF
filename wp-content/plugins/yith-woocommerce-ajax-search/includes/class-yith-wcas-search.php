...
if ( ! class_exists( 'YITH_WCAS_Search' ) ) {
	...
	class YITH_WCAS_Search {
		...
		public function pre_get_posts( $query ) {
			...
			if (( ! empty( $this->search_string ) || $fiilter_taxonomy) && ! is_admin() && ! ( defined( 'DOING_AJAX' ) && DOING_AJAX ) ) {
				if ( $fiilter_taxonomy ) {
					...
					if( $taxonomy ){
						$results = YITH_WCAS_Data_Search_Engine::get_instance()->search( $fiilter_taxonomy, $search_on, 0, $lang, false, 0, true );
					...
					$results = YITH_WCAS_Data_Search_Engine::get_instance()->search( $this->search_string, $search_on, 0, $lang, false, 0, true );
		...
		public function api_search( $request ) {
			...
			$results = YITH_WCAS_Data_Search_Engine::get_instance()->search( $query, $search_on, $category, $lang, true, $num_of_results, true );
			...
			if ( ! empty( $related_content ) ) {
				$results['related_content'] = YITH_WCAS_Data_Search_Engine::get_instance()->search( $query, $related_content, $category, $lang, true, $num_of_results, false );
			}
